...
static RTCDRV_TimerID_t sleepDelayTimerId;
static volatile bool delayComplete = false;
...
/**************************************************************************//**
 * @brief Callback used for sleep delay
 *****************************************************************************/
void sleepDelayCallback(RTCDRV_TimerID_t id, void *user)
{
  (void) user;
  (void) id;
  delayComplete = true;
...
s16 Si114xProcessIrq2(HANDLE si114x_handle, u16 timestamp, u16 *p_sample)
{
  ...
  sample.sequence  = irq_sequence;                                                   // sequence number
  sample.timestamp = timestamp;                                                      // 16-bit Timestamp to record
  sample.pad       = 0;
  sample.irqstat   = data_buffer[0];                                                 // 8-bit irq status
  sample.vis       = (((u16) (data_buffer[2]) << 8) & 0xff00) | data_buffer[1];      // VIS
...
static void startSleepTimer (uint32_t ms)
{
	RTCDRV_AllocateTimer(&sleepDelayTimerId);
	delayComplete = false;
	RTCDRV_StartTimer(sleepDelayTimerId, rtcdrvTimerTypeOneshot,
			ms, sleepDelayCallback, NULL);

}

static void stopSleepTimer()
{
	RTCDRV_FreeTimer(sleepDelayTimerId);
}
...
/**************************************************************************//**
 * @brief 1ms delay required by Si114x polling sequence. Leave blank
