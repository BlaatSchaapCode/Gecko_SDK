/***************************************************************************//**
 * @file
 * @brief LESENSE demo for EFM32_TGxxx_STK
 * @version 5.6.1
 *******************************************************************************
 * # License
...
#define LIGHTSENSE_SENSOR_PIN    4U

#define LIGHTSENSE_BUTTON0_PORT  gpioPortD
#define LIGHTSENSE_BUTTON0_PIN   8U
#define LIGHTSENSE_BUTTON0_FLAG  0x100
    ...
    /* MODE1, can be set by pressing PB0 on Tiny STK. */
      ...
      switch (appStateGlobal) {
        ...
        case SENSE_PREPARE_STATE:
        {
          ...
          PCNT_TopSet(PCNT0, LIGHTSENSE_NUMOF_EVENTS - 1U);
          /* Go to SENSE state. */
          appStateGlobal = SENSE_STATE;
        }
        break;
...
void LESENSE_IRQHandler(void)
{
  ...
  if (LESENSE_IF_CH4 & LESENSE_IntGetEnabled()) {
    LESENSE_IntClear(LESENSE_IF_CH4);
  }
...
void GPIO_EVEN_IRQHandler(void)
{
  ...
  if (++appModeGlobal >= LIGHTSENSE_NUMOF_MODES) {
    appModeGlobal = MODE0;
  }
