...
{
  Si72xx_ReadMagFieldDataAndSleep(SI7210_CONFIG_I2C_DEVICE,
                                  (SI7210_CONFIG_I2C_BUS_ADDRESS << 1),
                                  SI7210_200MT,
...
void randomColor(uint32_t * rngResult, RGBLED_Settings_t * ledSettings)
{
  ...
  // Convert the 128-bit number to two color values
  tmp = rngResult[0] ^ rngResult[1];
  ledSettings->led0Color = (tmp / 11930464) - 1;

  tmp = rngResult[2] ^ rngResult[3];
  ledSettings->led1Color = (tmp / 11930464) - 1;
}
