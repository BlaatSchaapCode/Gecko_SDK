...
{
  uint16_t cluster;
  cluster = entry[DIR_ENTRY_STARTCLUST_OFFS];
  cluster |= entry[DIR_ENTRY_STARTCLUST_OFFS + 1] << 8;
...
{
  uint32_t fileSize;
  fileSize = entry[DIR_ENTRY_FILESIZE_OFFS];
  fileSize |= entry[DIR_ENTRY_FILESIZE_OFFS + 1] << 8;
  fileSize |= entry[DIR_ENTRY_FILESIZE_OFFS + 2] << 16;
  fileSize |= entry[DIR_ENTRY_FILESIZE_OFFS + 3] << 24;
                                  ...
                                  uint8_t *flashPageBuf)
{
  ...
  bootloader_eraseRawStorage((uint32_t)diskStorage, flashPageSize);
  bootloader_eraseRawStorage((uint32_t)diskStorage + flashPageSize, flashPageSize);
