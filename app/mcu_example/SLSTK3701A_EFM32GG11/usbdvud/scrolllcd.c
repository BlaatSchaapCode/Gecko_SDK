...
#define BYTES_PER_LINE  (LS013B7DH06_WIDTH * LS013B7DH06_BITS_PER_PIXEL / 8)
               ...
               const char *pNewImg)
{
  ...
  if ( direction == scrollLeft ) {
    ...
    for ( step = 0; step <= BYTES_PER_LINE; step++ ) {
      ...
      for ( y = 0; y < LS013B7DH06_HEIGHT; y++ ) {
    ...
    for ( step = 0; step <= LS013B7DH06_HEIGHT; step += 2 ) {
      if ( direction == scrollUp ) {
        pLine     = &pOldImg[BYTES_PER_LINE * step];
        crossover = LS013B7DH06_HEIGHT - step;
        pNewLine  = pNewImg;
      } else {
        pLine     = &pNewImg[BYTES_PER_LINE * (LS013B7DH06_HEIGHT - step)];
        crossover = step;
        pNewLine  = pOldImg;
      }
