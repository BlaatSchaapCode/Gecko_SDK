...
#define LCSENSE_SENSOR_PIN   6U

#define LCSENSE_BUTTON_PORT  gpioPortD
#define LCSENSE_BUTTON_PIN   8U
#define LCSENSE_BUTTON_FLAG  0x100
    ...
    if (appModeGlobal == MODE0)
    {
      switch(appStateGlobal)
      {
        case BUTTON_PRESS_STATE:
        {
          ...
          /* Wait until SYNCBUSY_CTRL flag is cleared from the last write. */
        ...
        case SENSE_PREPARE_STATE:
        {
          ...
          LCD_Enable(false);
          /* Wait until SYNCBUSY_CTRL flag is cleared before entering EM2 in SENSE_STATE. */
          LCD_SyncBusyDelay(LCD_SYNCBUSY_CTRL);
