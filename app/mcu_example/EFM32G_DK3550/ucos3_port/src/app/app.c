...
static void App_TaskStart(void *p_arg)
{
  ...
  /* Write welcome message on serial(UART0)              */
  printf("\n*****************************************************************************");
  printf("\n                    uC/OS-III v%d.%02d.%02d on Silicon Labs EFM32 DK3550          ",
  ...
  printf("\nTask2: Receives characters from serial (UART0) and posts message to Task3");
  printf("\nTask3: Receives message from Task2 and writes it on LCD and serial (UART0)");
  printf("\n*****************************************************************************\n");
  printf("\nStart typing...\n");
