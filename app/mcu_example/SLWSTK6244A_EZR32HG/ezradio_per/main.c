...
/* Enable this to print all transmission related messages to the LCD.
 * Note: only PER and RSSI related messages are printed by default. */
//#define APP_ENABLE_ALL_TRX_PRINT
...
static volatile uint16_t appRxPktCntr = 0;
...
static void appPacketReceivedCallback ( EZRADIODRV_Handle_t handle, Ecode_t status )
{
  ...
  if ( status == ECODE_EMDRV_EZRADIODRV_OK )
  {
    ...
    {
#if (defined APP_ENABLE_ALL_TRX_PRINT)
      printf("-->Data RX: ACK\n");
#endif
    }
    else
    {
      ...
      if (rxData)
      {
        perVal = (uint8_t) ((((float)rxData - (float)appRxPktCntr) / (float)rxData) * 100.0);
      }

      /* Increase received data counter */
      appRxPktCntr++;
      ...
      printf("-->Data RX: %05d\n", rxData);
#endif
      printf("-->PER:%03d%% RSSI:%02d\n", perVal, ezradioReply.GET_MODEM_STATUS.LATCH_RSSI);
    }
  }
}
