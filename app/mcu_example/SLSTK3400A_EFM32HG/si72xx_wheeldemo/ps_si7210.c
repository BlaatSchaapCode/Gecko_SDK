...
void PS_SI7210_runMagneticFieldDemo(void)
{
  ...
  magField = Si72xx_ConvertDataCodesToMagneticField(mTScale, magData);
...
void PS_SI7210_runTemperatureDemo(void)
{
  ...
  if (!demoInitiated) {
    ...
    Si72xx_ReadTempCorrectionDataAndSleep(I2C0,
                                          SI7200_ADDR_0,
                                          &tempOffsetValue,
                                          &tempGainValue);
    demoInitiated = true;
  ...
  i2cError = Si72xx_ReadCorrectedTempAndSleep(I2C0,
                                              SI7200_ADDR_0,
                                              tempOffsetValue,
                                              tempGainValue,
                                              &mCTemp);
  ...
  if (!i2cError) {
    ...
    for (j = 1; j < SAMPLES_TO_AVG; j++) {
      i2cError = Si72xx_ReadTemperatureAndSleep(I2C0, SI7200_ADDR_0, &mCTemp);
...
{
  Si72xx_EnterSleepMode(I2C0, SI7200_ADDR_0, SI72XX_SLTIMEENA_MODE);
}
...
{
  Si72xx_EnterSleepMode(I2C0, SI7200_ADDR_0, SI72XX_SLEEP_MODE);
}
